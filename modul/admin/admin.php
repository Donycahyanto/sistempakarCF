...
function Blank_TextField_Validator_Cari()
{
  var form = document.forms['text_form'];
  if (!form) return true;
  if (form.keyword && form.keyword.value.trim() == "")
  {
...
switch($act){
  ...
  $tampil=mysqli_query($conn,"SELECT * FROM admin ORDER BY username");
	echo "<br><form method=POST action='?module=admin' name=text_form onsubmit='return Blank_TextField_Validator_Cari()'>
		  ...
		  <tr><td><input class='btn bg-olive margin' type=button name=tambah value='Tambah Admin' onclick=\"window.location.href='admin/tambahadmin';\">
  ...
  <input class='btn bg-olive margin' type=submit value='   Cari   ' name=Go></td> </tr>
	...
	if ($go_submit){
			$numrows = mysqli_num_rows(mysqli_query($conn,"SELECT * FROM admin WHERE username LIKE '%$keyword_esc%'"));
			if ($numrows > 0){
  ...
  case "editadmin":
    // Gunakan isset() untuk memastikan $_GET['id'] ada dan gunakan mysqli_real_escape_string 
    // untuk menghindari SQL Injection (Penting!)
    ...
    if (!empty($id_admin)) {
        ...
        if ($r=mysqli_fetch_array($edit)) {
            ...
            // Tampilkan pesan error jika admin tidak ditemukan
            echo "<div class='alert alert-danger'>Admin dengan username '$id_admin' tidak ditemukan.</div>";
        }
    } else {
        // Tampilkan pesan error jika ID tidak disediakan
